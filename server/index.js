...
async function resolveSpecUrl(inputUrl) {
  ...
  for (const c of candidates) {
    try {
      const resp = await fetch(c, { headers: { Accept: 'application/json' } });
      if (!resp.ok) continue;
      const j = await resp.json();
      if (j.openapi || j.swagger || j.paths) return c;
    } catch {}
  }
  try {
    ...
    if (urlMatch) {
      ...
      return specPath.startsWith('http') ? specPath : `${base}${specPath.startsWith('/') ? '' : '/'}${specPath}`;
    }
  } catch {}
  return url;
}
  ...
  try {
    ...
    if (pathname === '/api/urls') {
      ...
      if (req.method === 'POST') {
        ...
        await kv.updateOne({ _id: 'urls' }, { $set: { value: body } }, { upsert: true });
        return sendJson(res, { ok: true });
      }
    }
    ...
    if (pathname === '/api/global-params') {
      ...
      if (req.method === 'POST') {
        const body = await parseBody(req);
        await kv.updateOne({ _id: 'global-params' }, { $set: { value: body } }, { upsert: true });
        return sendJson(res, { ok: true });
      }
    }
    ...
    if (pathname === '/api/swagger-params') {
      if (req.method === 'GET') {
        const doc = await kv.findOne({ _id: 'swagger-params' });
        return sendJson(res, doc?.value ?? {});
      }
      if (req.method === 'POST') {
        const body = await parseBody(req);
        await kv.updateOne({ _id: 'swagger-params' }, { $set: { value: body } }, { upsert: true });
        return sendJson(res, { ok: true });
      }
    }
    ...
    if (pathname === '/api/scans') {
      ...
      if (req.method === 'GET') {
        ...
        return sendJson(res, docs.map(({ _id, createdAt, ...rest }) => rest));
      }
      if (req.method === 'POST') {
        const body = await parseBody(req);
    ...
    if (pathname === '/api/checkpoints' || pathname.startsWith('/api/checkpoints/')) {
      ...
      if (req.method === 'GET') {
        const docs = await col.find({}, { sort: { createdAt: 1 } }).toArray();
        return sendJson(res, docs.map(({ _id, createdAt, endpoints, definitions, ...meta }, i) => ({
          ...meta, index: i, endpointCount: (endpoints || []).length,
        })));
      }
      if (req.method === 'POST') {
        const body = await parseBody(req);
    ...
    if (pathname === '/api/requests' || pathname.startsWith('/api/requests/')) {
      ...
      if (req.method === 'GET' && !isById) {
        ...
        return sendJson(res, docs.map(({ _id, ...rest }) => rest));
      }
      if (req.method === 'POST' && !isById) {
        const body = await parseBody(req);
        const id = `${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
        const entry = {
          id, key: body.key, label: body.label || '', apiTitle: body.apiTitle || '',
          method: body.method, path: body.path, fullUrl: body.fullUrl,
          payload: body.payload || null, pathOverrides: body.pathOverrides || {},
          swaggerParams: body.swaggerParams || {}, response: body.response || null,
          pinned: body.pinned || false, savedAt: new Date().toISOString(),
        };
    ...
    if (req.method === 'GET' && pathname === '/api/fetch-spec') {
      ...
      try { data = JSON.parse(text); } catch {
        return sendJson(res, { error: `URL did not return valid JSON. Resolved: ${specUrl}` }, 400);
      }
      if (!data.openapi && !data.swagger && !data.paths) {
        return sendJson(res, { error: `Not an OpenAPI/Swagger spec. Resolved: ${specUrl}` }, 400);
      }
      return sendJson(res, data);
    }
...
function convertToPostman(name, scan) {
  ...
  for (const ep of scan.endpoints) {
    ...
    const item = {
      ...
      request: {
        method: ep.method.toUpperCase(), header: [],
    ...
    if (ep.requestBody) {
      const jc = ep.requestBody.content?.['application/json'];
      if (jc) {
        item.request.body = { mode: 'raw', raw: JSON.stringify(jc.schema || {}, null, 2), options: { raw: { language: 'json' } } };
