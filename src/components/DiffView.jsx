...
function renderDiff(diff, newApiGroups, removedApiGroups, plainAdded, plainRemoved, expandedDiff, setExpandedDiff) {
  ...
  // Group plainAdded, plainRemoved, and changed by _apiTitle
  const plainAddedGroups = {};
  const plainRemovedGroups = {};
  const changedGroups = {};

  for (const ep of plainAdded) {
    const title = ep._apiTitle || 'Unknown API';
    if (!plainAddedGroups[title]) plainAddedGroups[title] = [];
    plainAddedGroups[title].push(ep);
  }

  for (const ep of plainRemoved) {
    const title = ep._apiTitle || 'Unknown API';
    if (!plainRemovedGroups[title]) plainRemovedGroups[title] = [];
    plainRemovedGroups[title].push(ep);
  }

  for (const c of diff.changed) {
    const title = c.endpoint._apiTitle || 'Unknown API';
    if (!changedGroups[title]) changedGroups[title] = [];
    changedGroups[title].push(c);
  }
  ...
  return (
    <>
      ...
      {Object.entries(plainAddedGroups).map(([title, eps]) => (
        <div key={title} className="diff-api-section mb-4">
          <div className="diff-api-section-header">
            <StateBadge state={DIFF_STATE.NEW} />
            <span className="diff-api-section-title">{title}</span>
            <span className="text-sm text-muted">{eps.length} added endpoint{eps.length !== 1 ? 's' : ''}</span>
          </div>
          {eps.map((ep, i) => (
            <EpRow key={i} ep={ep} diffKey={`add:${title}:${i}`} expandedDiff={expandedDiff} setExpandedDiff={setExpandedDiff} state={DIFF_STATE.NEW}>
              <div className="diff-side-by-side">
                <div className="diff-side diff-side-new" style={{ gridColumn: '1 / -1' }}>
                  <div className="diff-side-label">New endpoint</div>
                  {endpointSnapshot(ep).map((line, j) => (
      ...
      {Object.entries(plainRemovedGroups).map(([title, eps]) => (
        <div key={title} className="diff-api-section mb-4">
          <div className="diff-api-section-header">
            <StateBadge state={DIFF_STATE.REMOVED} />
            <span className="diff-api-section-title">{title}</span>
            <span className="text-sm text-muted">{eps.length} removed endpoint{eps.length !== 1 ? 's' : ''}</span>
          </div>
          {eps.map((ep, i) => (
            <EpRow key={i} ep={ep} diffKey={`rem:${title}:${i}`} expandedDiff={expandedDiff} setExpandedDiff={setExpandedDiff} state={DIFF_STATE.REMOVED}>
              <div className="diff-side-by-side">
                <div className="diff-side diff-side-old" style={{ gridColumn: '1 / -1' }}>
                  <div className="diff-side-label">Removed endpoint</div>
                  {endpointSnapshot(ep).map((line, j) => (
      ...
      {Object.entries(changedGroups).map(([title, changes]) => (
        <div key={title} className="diff-api-section mb-4">
          <div className="diff-api-section-header">
            <StateBadge state={DIFF_STATE.UPDATED} />
            <span className="diff-api-section-title">{title}</span>
            <span className="text-sm text-muted">{changes.length} changed endpoint{changes.length !== 1 ? 's' : ''}</span>
          </div>
          {changes.map((c, i) => {
            const diffKey = `chg:${title}:${i}`;
