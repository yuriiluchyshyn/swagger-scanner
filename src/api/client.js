...
export function setStoredEmail(email) {
  localStorage.setItem('open-api-scanner-email', email.toLowerCase());
}
...
export async function executeRequest({ url, method, headers, body }, corsSettings = {}) {
  const globalMode = corsSettings.globalMode || 'browser-first';
  const blockedDomains = corsSettings.blockedDomains || [];
  ...
  if (shouldSkipBrowser) {
    console.log('Skipping browser request due to CORS settings, using server proxy...');
    try {
  ...
  // Try direct browser request first (like Postman web and Swagger UI)
  try {
    console.log('Attempting direct browser request...');
    const opts = { 
      method: method.toUpperCase(), 
      headers: headers || {}
      // Removed mode: 'cors' to avoid preflight requests like Swagger UI does
    };
    ...
    try {
      ...
      data = await resp.text(); // Fallback to text
    }
    
    console.log('✓ Direct browser request successful');
    return {
      status: resp.status,
    ...
    try {
      ...
      // If both methods fail, throw a comprehensive error
      throw new Error(`Both browser and server request failed. Browser: ${directError.message}. Server: ${serverError.message}`);
    }
  }
}
